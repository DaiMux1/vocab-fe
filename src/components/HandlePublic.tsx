...
function HandlePublic() {
	...
	const handleRequestPublic = async (state: number) => {
		try {
			await handleReqPublic(state, id);
		...
		<Container maxWidth="xl">
			<Box mx={10}>
				...
				<Box display={'flex'} alignSelf="center" alignItems={'center'}>
					...
					<Box mx={1}></Box>
					<Button variant="contained" onClick={() => handleRequestPublic(2)}>
						Duyệt
					</Button>
					<Box mx={1}></Box>
					<Button
						variant="contained"
						onClick={() => handleRequestPublic(1)}
						sx={{ bgcolor: 'red' }}
					>
						Không duyệt
					</Button>
